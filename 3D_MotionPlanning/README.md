...
## -->Step 6 Solution:
- The main difference is that in backyard_flyer, the drone was programmed to fly at a certain altitude and then fly through 4 points that form a square. 
However in motion_planning.py is kind of the same except that this time a list of obstacles is imported with some safety margin north and east. 
In addition a path through these obstacles is planned using a_star algorithm and drone is flying along that path in a zigzag motion.
